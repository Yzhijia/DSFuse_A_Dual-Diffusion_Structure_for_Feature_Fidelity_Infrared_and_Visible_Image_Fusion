...
class GaussianDiffusion(nn.Module):
    def __init__(self, denoise_fn_ir, denoise_fn_vi, encode_fusion, timesteps=1000, loss_type='l1'):
        super().__init__()
        self.denoise_fn_ir = denoise_fn_ir
        self.denoise_fn_vi = denoise_fn_vi
    ...
    def forward(self, img_ir, img_vi, mask_img, t=None, *args, **kwargs):
        ...
        # print(x.shape)
        # print(x.shape)
        cond = self.encode_fusion(torch.cat([img_ir, img_vi_gray], dim=1))

        cond_x = cond
        # x = img_ir
        # print("testing........change img_ir as cond........................")

        # cond = img_ir
        # res = True
        if self.res:
            x = self.img2res(img_ir, cond_x)
            # print("testing.................res=true................")
        else:
            x = img_ir
        ...
        p_losses_ir, x_tp1, noise_pred, x_t, x_t_gt, x_0 = self.p_losses(x, t, cond_x, self.denoise_fn_ir, *args, **kwargs)
        # p_losses_ir = 0

        # x = img_vi_gray

        if self.res:
            x = self.img2res(img_vi_gray, cond_x)
        else:
            x = img_vi_gray

        # print(t)
        # print("*****************只训练ir diffusion")
        # p_losses_vi = 0
        p_losses_vi, x_tp1, noise_pred, x_t, x_t_gt, x_0 = self.p_losses(x, t, cond_x, self.denoise_fn_vi, *args, **kwargs)
        ...
        # total_loss = 0
        all_loss = (p_losses_ir+p_losses_vi)*hparams["p_diff"] + total_loss*0
        # print("no_vi")
        ret = {'q': all_loss, 'q_ir':p_losses_ir,'q_vi':p_losses_vi, 'fusion_loss': total_loss}
        ...
        if self.res:
            x_tp1 = self.res2img(x_tp1, cond_x)
            x_t = self.res2img(x_t, cond_x)
            x_t_gt = self.res2img(x_t_gt, cond_x)
            # x_tp1 = cond
            # x_t = cond
            # x_t_gt = cond
    ...
    @torch.no_grad()
    def sample(self, img_ir, img_vi, shape, save_intermediate=False):
        device = self.betas.device
        x = torch.cat([img_ir, img_vi], dim=1)
        # print(x.shape)
        b = shape[0]
        # shape[1]=2
        img_ir_r = torch.randn(shape, device = device)
        ...
        images_irs = []
        for i in it:
            img_ir_r, x_recon_ir = self.p_sample(
                img_ir_r, torch.full((b,), i, device=device, dtype=torch.long), cond_x, self.denoise_fn_ir)
            if save_intermediate:
                if self.res:
                    imgIR_ = self.res2img(img_ir_r, cond_x)
                    x_reconIR_ = self.res2img(x_recon_ir, cond_x)
                else:
                    imgIR_ = img_ir_r
                    x_reconIR_ = x_recon_ir
                images_irs.append((imgIR_.cpu(), x_reconIR_.cpu()))
        if self.res:
            img_ir_r = self.res2img(img_ir_r, cond_x)
        else:
            img_ir_r = img_ir_r
        ...
        for m in it_vi:
            img_vi_r, x_recon_vi = self.p_sample(
                img_vi_r, torch.full((b,), m, device=device, dtype=torch.long), cond_x, self.denoise_fn_vi)
            if save_intermediate:
                imgVI_ = img_vi_r
                x_reconVI_ = x_recon_vi
                images_vis.append((imgVI_.cpu(), x_reconVI_.cpu()))
        if self.res:
            img_vi_r = self.res2img(img_vi_r, cond_x)
        else:
            img_vi_r = img_vi_r

        if save_intermediate:
            return (img_ir_r, img_vi_r), cond, (images_irs, images_vis)
        else:
            return (img_ir_r, img_vi_r), cond

    @torch.no_grad()
    def interpolate(self, x1, x2, img_ir, img_vi, t=None, lam=0.5):
