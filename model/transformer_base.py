...
class Cross_WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.,
        ...
        self.qkv = nn.Linear(dim, dim * 3, bias=False)
        self.q = nn.Linear(dim, dim, bias=None)
        self.kv = nn.Linear(dim, dim*2, bias=None)
        if qkv_bias:
            self.q_bias = nn.Parameter(torch.zeros(dim))
            self.v_bias = nn.Parameter(torch.zeros(dim))
        else:
    ...
    def forward(self, x, y, mask=None):
        ...
        if self.q_bias is not None:
            qkv_bias = torch.cat((self.q_bias, torch.zeros_like(self.v_bias, requires_grad=False), self.v_bias))
        q = F.linear(input=x, weight=self.q.weight, bias=qkv_bias)
        kv = F.linear(input=y, weight=self.kv.weight, bias=qkv_bias)
        # qkv = F.linear(input=x, weight=self.qkv.weight, bias=qkv_bias)
        q = q.reshape(B_, N, 1, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        kv = kv.reshape(B_, N, 1, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        # q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        q, k, v = q[0], kv[0], kv[1]
...
class Cross_SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, x_size):
        ...
        if self.input_resolution == x_size:
            ...
            attn_windows_B = self.attn_B(y_windows, x_windows, mask=self.calculate_mask(x_size).to(x.device))
        ...
        x = x.view(B, H * W, C)
        y = y.view(B, H * W, C)
        x = shortcut_A + self.drop_path_A(self.norm1_A(x))
        y = shortcut_B + self.drop_path_B(self.norm1_B(y))
        # FFN
        x = x + self.drop_path_A(self.norm2_A(self.mlp_A(x)))
        y = y + self.drop_path_B(self.norm2_B(self.mlp_B(y)))
        return x, y
...
class CRSTB(nn.Module):
    ...
    def forward(self, x, y, x_size):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, y, x_size), x_size)))
