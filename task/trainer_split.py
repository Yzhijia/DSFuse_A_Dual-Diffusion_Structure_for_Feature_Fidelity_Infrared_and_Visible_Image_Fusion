...
from utils.utils import plot_img, move_to_cuda, load_checkpoint_total, save_checkpoint_total, tensors_to_scalars, load_ckpt, Measure, load_checkpoint_total_s, save_checkpoint_total_s
...
class Trainer:
    ...
    def train(self):
        ...
        model_ir, model_vi, model_fusion = self.build_model()
        optimizer_d_ir = self.build_optimizer(model_ir)
        optimizer_d_vi = self.build_optimizer(model_vi)
        optimizer_f = self.build_optimizer(model_fusion)
        # optimizer_d = self.build_optimizer(model)
        print("载入或初始化全部的参数******************************************************分开载入参数")
        self.global_step = training_step = load_checkpoint_total_s(model_ir,model_vi,model_fusion, optimizer_d_ir, optimizer_d_vi, optimizer_f, hparams['work_dir'])
            ...
            for batch in train_pbar:
                ...
                if opt["rank"] == 0:
                    if training_step % hparams['val_check_interval'] == 0:
                        ...
                        save_checkpoint_total_s(model_ir, model_vi, model_fusion, optimizer_d_ir, optimizer_d_vi, optimizer_f, self.work_dir,
                ...
                fusionloss, losses_fusion = self.training_step_fusion(batch)
                ...
                losses_diffusion, dif_loss = self.training_step_diffusion(batch)
                # print(losses)
                dif_loss.backward()
                ...
                losses = losses_diffusion.copy()
                losses.update(losses_fusion)
                # optimizer_f.zero_grad()
                # optimizer_d_vi.zero_grad()
    ...
    def test(self):
        ...
        load_checkpoint_total_s(model_ir, model_vi, model_fusion, optimizer_d_ir, optimizer_d_vi, optimizer_f, hparams['work_dir'])
        # load_checkpoint_total(model, optimizer_d, hparams['work_dir'])
        optimizer = None
        device_id = torch.cuda.current_device()
        # 用最近的
        # self.global_step
        ...
        self.model_ir.sample_tqdm = False

        torch.backends.cudnn.benchmark = False
        if hparams['test_save_png']:
            if hasattr(self.model_vi, 'make_generation_fast_'):
                self.model_vi.make_generation_fast_()
            if hasattr(self.model_ir, 'make_generation_fast_'):
                self.model_ir.make_generation_fast_()
...
if __name__ == '__main__':
    set_hparams()
    print("损失函数参数\n ir pixel:"+str(hparams['p1'])+
          "\n vi pixel:"+str(hparams['p2'])+"\n ir gradient:"+str(hparams['p3'])+
          "\n vi gradient:"+str(hparams['p4'])+"\n max gradient:"+str(hparams['p5'])+
          "\n ssim:"+str(hparams['p6']))
    ...
    trainer = task.fusiondiff.FusionDiffTrainer_Split()
